...
function TicTacToeOnePlayer() {
  const emptyBoard = ["", "", "", "", "", "", "", "", ""];
  const [board, setBoard] = useState(emptyBoard);
  const [currentPlayer, setCurrentPlayer] = useState("X");
  const [winner, setWinner] = useState(null);

  const userPlayer = (index) => {
        ...
        <div class={`board ${winner ? "game-over" : ""}`}>
          {board.map((item, index) => (
            <div
              key={index}
              className={`cell ${item}`}
              onClick={() => userPlayer(index)}
            >
              {winner ? "" : currentPlayer === "O" ? robotPlayer(index): ""}
              {item}
            </div>
          ))}
        </div>
...
export default TicTacToeOnePlayer;
